...
export const helpEn = {
        ...
        useBackendChatJobs: "If enabled, the actual AI request is executed by the PocketRisu-Alter server. Generation continues even if you turn off the screen or close the browser, and the completed message is loaded when you reopen the app. Currently OpenAI-compatible providers only.\n\nAuxiliary requests that run before generation (HypaMemory embedding/summarization, translation, etc.) are also executed by the server, which holds the result — turning the screen off mid-request no longer fails. While a generation is running, a silent audio track keeps the browser from freezing the app in the background, so the pipeline keeps advancing (embedding → summarization → main generation) with the screen off. Local (in-browser) embedding models are not guaranteed to run in the background, so OpenAI or an OpenAI-compatible (Custom) embedding is recommended.",
