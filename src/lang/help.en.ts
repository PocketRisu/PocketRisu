...
export const helpEn = {
        ...
        useBackendChatJobs: "If enabled, the actual AI request is executed by the PocketRisu-Alter server. Generation continues even if you turn off the screen or close the browser, and the completed message is loaded when you reopen the app. Currently OpenAI-compatible providers only.\n\nWith HypaMemory (HypaV3), the whole memory step — summarization, embedding, and memory assembly — runs inside the same server job as the main generation. The server owns the entire turn from the moment you hit send: turning the screen off or closing the app no longer interrupts summarization or generation, and new summaries are saved with the result. This requires an OpenAI or OpenAI-compatible (Custom) embedding model and an OpenAI-compatible summarization (aux) model — otherwise the memory step falls back to running in the app as before. Other auxiliary requests (translation, etc.) are also executed and held by the server, so they survive the screen turning off.",
